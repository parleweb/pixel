...
def resizeImage(newHeight, image):
    width, height = image.size
    ratio = width / float(height)
    newWidth = int(newHeight * ratio)
    resized = image.resize((newWidth, newHeight), PIL.Image.LANCZOS)
    imgByteArr = io.BytesIO()
    resized.save(imgByteArr, format="JPEG")
    imgByteArr = imgByteArr.getvalue()
    return imgByteArr

def addImageS3(data):
    bucket = os.getenv("AWS_BUCKET", None)
    rectoData = cacheImage(data["rectoUrl"])
    versoData = cacheImage(data["versoUrl"])
    image = Image.open(io.BytesIO(rectoData))
    menu = resizeImage(70, image)
    xs = resizeImage(320, image)
...
class PlwMedia(object):
    ...
    def copyfile(self, foldername="default", sourcedir=".", targetdir="web", scanfor = '.jpg', scanoption = '@all', jsonfile = "copyfile.json"):
        if( targetdir[-1] != '\\' ):
            targetdir += "\\"
    ...
    def scanimage(self, ffoldername="default", sourcedir=".", targetdir="web", resizeimg = 2.5, resizeimgth = 10, scanfor = '.jpg', scanoption = '@files', jsonfile = "scanimage.json"):
    ...
    def scanmedia(self, foldername="default", sourcedir=".", targetdir="web", resize = [], resizelen = 0, scanfor = '.jpg', scanoption = '@files', jsonfile = "scanimage.json"):
        if( resizelen == 0 ):
            resize = self.resize
                ...
                for filename in files:
                    ...
                    if ext:
                        ...
                        if( isScanOnlyfiles == -1 ):
                            ...
                            newfile = targetdir+subdir+filenamenoext+'.web'+ext
                            newfileth = targetdir+subdir+filenamenoext+'.th'+ext                            
                            logger.debug("dir "+dirpath + " subdir "+subdir+" file "+filename)
                        else:
                            newfile = targetdir+filenamenoext+'.web'+ext
                            newfileth = targetdir+filenamenoext+'.th'+ext
...
cwd = os.getcwd()
PlwMedia().scanmedia(sourcedir='photo/yne/20-1', targetdir='photo.web/yne/20-1')
