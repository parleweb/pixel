                            ...
                            if( filename not in activedatafile or dirpath != activedatadir ):
                                if filename.rfind(scanfor) != -1:
                                    ...
                                    if( True ): #filetobuild != activedatafile ):
                                        logger.info("#")
                                        logger.info("build : " + filetobuild)

                                        try:
                                            if( dirpath[:-1] != '\\' ):
                                                dirpath_endslash = dirpath + '\\'
                                            else:
                                                dirpath_endslash = dirpath
                                            currentfilename = (dirpath_endslash + filename).lower()

                                            logger.info('current filename is : '+currentfilename)
                                            id = scanobj.urllist[currentfilename]
                                            logger.info('current filename id is : '+id)
                                        except:
                                            pass
